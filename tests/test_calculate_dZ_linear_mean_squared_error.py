...
def test_calculate_dZ_linear_mean_squared_error():
    ...
    m = A.shape[1]
    expected_output = 2 * (A - Y) / m
    print("\nExpected output (manual calculation):")
    print(expected_output)
    
    # Calculate dZ using the function
    result = calculate_dZ_linear_mean_squared_error(A, Y, Z)
    print("\nActual output (calculate_dZ_linear_mean_squared_error function):")
    print(result)
    ...
    Z_2 = A_2  # For linear activation, Z equals A
    result_2 = calculate_dZ_linear_mean_squared_error(A_2, Y_2, Z_2)
    ...
    print("\nCalculate dZ for Linear with mean squared error function test passed!")
